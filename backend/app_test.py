"""
Test version of Verba Backend - Uses mock transcriber for testing without Whisper
"""
...
# Import our modules - use mock transcriber for testing
from transcriber_mock import transcribe_audio  # Mock version
...
app = FastAPI(title="Verba API (Test Mode)", version="0.2.0-test", description="Offline-first meeting assistant - TEST MODE")
...
def root():
    ...
    return {
        "status": "ok",
        "message": "Verba API is running (TEST MODE - Mock transcription)",
        "version": "0.2.0-test"
    }
...
async def transcribe(audio: UploadFile = File(...)):
    """
    Transcribe audio file using mock transcriber (for testing)
    ...
    try:
        ...
        logger.info(f"Transcribing audio file: {audio.filename} ({len(content)} bytes) - MOCK MODE")
        
        # Transcribe with mock transcriber
        ...
        logger.info(f"Transcription successful (MOCK): {len(transcript)} characters")
...
"""
        ...
        markdown += "\n---\n\n*Generated by Verba - Offline-first meeting assistant (TEST MODE)*\n"
...
if __name__ == "__main__":
    logger.info("Starting Verba API server (TEST MODE)...")
    logger.info("⚠️  Using MOCK transcriber - no Whisper required")
